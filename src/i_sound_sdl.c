/*
 * i_sound_sdl.c - Copyright (c) 2024 - Olivier Poncet
 *
    ...
    /* create chunk if needed */ {
        ...
        if(chunk == NULL) {
            ...
            if(sound_format == 0x0003) {
                const uint32_t length = ((sound_length * audio_frequency) / sound_frequency);
                uint8_t*       buffer = (uint8_t*) SDL_malloc((length + 7) & ~7);
                /* check buffer */ {
                    if(buffer == NULL) {
                        I_Error("SDL_malloc has failed");
                    }
                }
                /* resample sound */ {
                    if(sound_frequency <= audio_frequency) {
                        uint8_t* srcbuf = ((uint8_t*)(sfx->data)) + 0x18;
                        uint8_t* dstbuf = buffer;
                        uint32_t dstlen = length;
                        uint16_t error  = 0;
                        if(dstlen != 0) {
                            do {
                                *dstbuf++ = (*srcbuf - 128);
                                error += sound_frequency;
                                if(error >= audio_frequency) {
                                    error -= audio_frequency;
                                    ++srcbuf;
                                }
                            } while(--dstlen != 0);
                        }
                        ...
                        I_Alert("unable to resample sound, to be implemented");
                    }
                }
                /* create chunk */ {
                    ...
                    if(chunk != NULL) {
                        chunk->allocated = 1;
                        chunk->volume    = ((volume << 4) | (volume >> 1));
